            ...
            {(() => {
              ...
              return (
                <div className="overflow-y-auto" style={{ maxHeight: 280 }}>
                  {statusRows.map(row => (
                    <div key={row.label} className="project-bar-row">
                      <div className="project-bar-label">{row.label}</div>
                      <div className="project-bar-track">
                        <div className="project-bar-fill" style={{ width: `${(row.count / maxVal) * 100}%`, background: row.color }} />
                      </div>
                      <div className="project-bar-count">{row.count}</div>
                    </div>
                  ))}
                </div>
              );
